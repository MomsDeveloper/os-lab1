...
void test_remove_duplicates(){
    int numbers_to_sort[] = {0, 0, 1, 0, 1, 2, 5, 4, 3, 0, 1, 2};
    int numbers_sorted[] = {0, 1, 2, 5, 4, 3};

    int kol = 12;
    remove_duplicates(numbers_to_sort, &kol);

    for (int i = 0; i < 6; i++) {
        if (numbers_to_sort[i] != numbers_sorted[i]) {
            printf("Deduplicate test failed\n");
            return;
        }
    }

    printf("* Deduplicate test passed\n");
}
